...
/** Windows build of the notebook editor: Ctrl-Enter submits (Cmd does not),
 *  the IME's own Enter is ignored, and mixed Chinese/English list
 *  continuation behaves identically to macOS.
 *
...
describe('Windows: Mod-Enter resolution', () => {
  ...
  it('plain Cmd-Enter does not submit', async () => {
    const { content, options, editor, host } = await mountEditor(editorModule, EditorView, '- 中文要点')
...
describe('Windows: Enter inside an open IME composition', () => {
...
describe('Windows: mixed Chinese/English list continuation', () => {
  it.each([
