/**
 * The global Cmd/Ctrl+E toggle shortcut. Hosted by the tool-row toggle (the
 * one registration that stays mounted for the whole session), registered on
 * `document` in the capture phase so it wins over both the CodeMirror
 * keymap and the browser before any focused surface can react.
 *
 ...
 * `metaKey | ctrlKey` chord with no shift/alt keeps it a single reversible
 * toggle (⌘E opens the notebook, ⌘E again hands focus back to the
 * composer), mirroring Notion/Obsidian's source-mode convention.
 */
...
/** Platform-correct shortcut label for tooltips (`⌘E` on Apple, `Ctrl+E` elsewhere). */
export function toggleShortcutLabel(): string {
  return /mac|iphone|ipad/i.test(navigator.userAgent) ? '⌘E' : 'Ctrl+E'
}
